...
-- Create function to recover stuck jobs
...
BEGIN
  ...
  FOR stuck_job IN
    ...
    WHERE (
      -- Processing jobs stuck for more than 2 minutes
      (status = 'processing' AND started_at < NOW() - INTERVAL '2 minutes')
      OR
      -- Failed jobs stuck for more than 2 minutes
      (status = 'failed' AND started_at < NOW() - INTERVAL '2 minutes')
    )
...
-- Schedule the recovery job to run every 2 minutes
...
GRANT EXECUTE ON FUNCTION recover_stuck_jobs() TO postgres;
GRANT EXECUTE ON FUNCTION recover_stuck_jobs() TO service_role;

-- Add comment for documentation
COMMENT ON FUNCTION recover_stuck_jobs() IS 'Recovers stuck processing jobs by resetting them to pending and triggering process-next-chunk';
