...
async function generatePdfReport(inspectionId) {
  try {
...
async function calculateTotalCostsFromJobs(inspectionId) {
  const { data: jobs } = await supabase.from("processing_jobs").select("cost, total_tokens, web_search_count, web_search_results").eq("inspection_id", inspectionId).eq("status", "completed");
...
serve(async (req)=>{
  try {
    ...
    const { data: finalChunkJob, error: finalChunkError } = await supabase.from("processing_jobs").select("chunk_result, sequence_order").eq("inspection_id", inspectionId).eq("job_type", "chunk_analysis").eq("status", "completed").order("sequence_order", {
      ascending: false
    }).limit(1).single();
    if (finalChunkError || !finalChunkJob || !finalChunkJob.chunk_result) {
      ...
      return new Response(JSON.stringify({
        error: "No final chunk result found"
      }), {
        status: 500,
        headers: {
          "Content-Type": "application/json"
    ...
    const { data: allJobs } = await supabase.from("processing_jobs").select("job_type, chunk_result, sequence_order").eq("inspection_id", inspectionId).eq("status", "completed").order("sequence_order", {
      ascending: true
    });
    ...
    const { data: inspection, error: inspectionError } = await supabase.from("inspections").select("id, vin, email").eq("id", inspectionId).single();
    if (inspectionError) {
    ...
    const { data: allChunks } = await supabase.from("processing_jobs").select("id").eq("inspection_id", inspectionId).eq("job_type", "chunk_analysis").eq("status", "completed");
