...
3. **Output Format – JSON:** After analysis, output **only** a single JSON object containing:
   ...
   - **A section for each image category** ('exterior', 'interior', 'dashboard', 'paint', 'rust', 'engine', 'undercarriage', 'obd', 'title'). Each of these is an object with:
     ...
     - 'incomplete': a boolean indicating if this category couldn't be fully assessed (e.g. missing/blurry images or data, or multiple conflicting vehicle data detected even if user attached one irrelevant image). Use 'true' if incomplete, otherwise 'false' or omit if fully assessed.
...
6. **Edge Cases:** Handle uncertainties or missing info as follows:
   ...
   - If multiple car images with conflicting data are provided, just use true for 'incomplete' field of the relevant category and focus analysis on the most relevant vehicle based on VIN or primary data provided in the DATA_BLOCK section. Do not attempt to merge unrelated vehicles into one report. When multiple vehicle data is detected, set 'incomplete': true for affected categories and provide 'incompletion_reason': "Multiple vehicle data detected" (or combine with other reasons using semicolons if applicable).
